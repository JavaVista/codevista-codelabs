((window, document) => {
  ...
  const app = () => {
    ...
    // Dark mode toggle function for Polymer on-tap event
    app.toggleDarkMode = function(e, detail) {
  ...
  const init = () => {
    ...
    // Dark Mode Toggle (robust: direct binding + event delegation)
    ...
    // Fallback: event delegation to capture clicks on inner elements/shadow DOM
    document.addEventListener('click', (e) => {
      const btn = e.target && e.target.closest && e.target.closest('#dark-mode-toggle');
      if (btn) toggleTheme();
    });
