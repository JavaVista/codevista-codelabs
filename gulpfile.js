...
gulp.task('build:js', (callback) => {
  ...
  // Copy scripts from app/scripts/ to build/scripts/
  const scriptSrcs = [
    'app/scripts/**/*',
  ];
...
/**
 * watch:css watches scss files for changes and re-builds them
 * (was incorrectly calling build:css which doesn't compile scss)
 */
gulp.task('watch:css', () => {
...
const generateView = () => {
  return through.obj((file, enc, callback) => {
    ...
    // e.g. "/" for default local, "/codevista-codelabs/" for GitHub Pages default
...
const viewFuncs = {
  ...
  categoryHasShowableCodelabs: (viewId) => {
    return (category, codelabs) => {
      const codelabsUsingCategory = codelabs
        .filter((codelab) => {
          return codelab.category && codelab.category.indexOf(category) !== -1;
        })
        .filter((codelab) => {
          // Filter hidden codelabs from the default view. All other views are
          // explicitly opt-in via metadata.
          const status = Array.isArray(codelab.status) ? codelab.status : (codelab.status ? [String(codelab.status).toLowerCase()] : ['published']);
          return viewId !== 'default' || !status || status.indexOf('hidden') === -1;
        });
  ...
  codelabPin: () => {
    return (view, codelab) => {
      ...
      const i = pins.indexOf(codelab.id);
      if (i >= 0) {
        return i + 1;
      }
      return '';
...
const filterCodelabs = (view, codelabs) => {
  ...
  codelabs = codelabs.filter(function(codelab) {
    ...
    if (!match || !view.exclude || !view.exclude.length) {
      return match;
    }
...
const sortCodelabs = (codelabs, view) => {
  ...
  codelabs.sort(function(a, b) {
    ...
    const pins = view.pins || [];
    var ia = pins.indexOf(a.id);
    var ib = pins.indexOf(b.id);
    if (ia >= 0 && ib < 0) {
      return -1;
    }
    if (ia < 0 && ib >= 0) {
